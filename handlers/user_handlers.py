...
@router.callback_query(
    F.data == callbacks[buttons["upcoming_events"]], IsNotRegistration()
)
async def start_button_handler(callback: CallbackQuery):
    ...
    if callback.message.photo:
        ...
        return await callback.message.answer(
            LEXICON["events_list"], reply_markup=kb.events_list(events)
        )
...
async def event_info_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer_photo(
        ...
        reply_markup=kb.register_to_event(
            event.id, show_registration, show_payment_confirmation
        ),
    )
...
async def cancel_registration_beer_pong_handler(callback: CallbackQuery):
    if callback.data.split("_")[-1] == "visitor":
        if await db.remove_registration(BEER_PONG_EVENT_ID, callback.from_user.id):
            await callback.message.delete()
            return await callback.message.answer(
                "<b>✅ Регистрация на мероприятие снята</b>", reply_markup=kb.start()
            )
        else:
...
async def register_for_the_event_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if not (user.name and user.date_of_birth):
        ...
        message = await callback.message.answer(
            text=LEXICON["you_need_to_sign_in"], reply_markup=kb.cancel_registration()
        )

        await state.set_state(UserState.sign_in_enter_name)
        return await state.update_data(
            registration_message_id=message.message_id, registration_to_event=event_id
        )
    ...
    if registration:
        ...
        return await callback.answer(
            "✅ Вы уже зарегистрированы на данное мероприятие", show_alert=True
        )
    ...
    try:
        ...
        print(
            f"Ошибка при попытке изменения подписи (register_for_the_event_handler): {e}"
        )
        ...
        try:
            ...
            if is_user_adult(user.date_of_birth):
                text = LEXICON["payment_instructions"].format(
                    ...
                    fundraiser.preferred_bank,
                    2,
                    fundraiser.username,
                )
            else:
                text = LEXICON["payment_instructions"].format(
                    LEXICON["underage_instruction"].format(fundraiser.username),
                    2,
                    fundraiser.phone_number,
                    fundraiser.preferred_bank,
                    3,
            ...
            await callback.message.answer(
                text=text, reply_markup=kb.confirm_payment(event.id)
            )
...
async def send_payment_confirmation(callback: CallbackQuery, state: FSMContext):
    ...
    if registration.status != "waiting_for_payment":
        ...
        await state.set_state(UserState.default_state)
        return await callback.answer(
            LEXICON["contact_your_fundraiser"], show_alert=True
        )
    ...
    await state.update_data(
        event_id=event_id,
        registration_id=registration.id,  # TODO: нужно добавлять это раньше
...
async def send_payment_confirmation_handler(message: Message, state: FSMContext):
    ...
    try:
        await bot.edit_message_reply_markup(
            ...
            message_id=data["last_payment_confirmation_message_message_id"],
            reply_markup=None,
        )
    except Exception as e:
    ...
    if message.photo:
        await bot.send_photo(
            chat_id=registration.fundraiser_id,
    ...
    await db.decrement_registration_and_increment_verification(
        registration.fundraiser_id
    )
    await db.update_registration_status(
        registration_id, "waiting_for_fundraiser_confirmation"
    )
    ...
    if len(name.split()) != 3:
        try:
            return await bot.edit_message_text(
                ...
                text=LEXICON[
                    "sign_in_enter_name_again"
                ],  # TODO: кнопку, если нет отчества
    ...
    if not validation_check["valid"]:
        try:
            return await bot.edit_message_text(
                chat_id=message.chat.id,
                message_id=data["registration_message_id"],
                text=LEXICON["sign_in_enter_date_of_birth_again"].format(
                    validation_check["reason"]
                ),
                reply_markup=kb.profile_registration_back_to_name(),
            )
        except TelegramBadRequest:
...
@router.callback_query(
    F.data.in_(
        [
            callbacks[buttons["registration_status_bachelor-cu"]],
            callbacks[buttons["registration_status_master-cu"]],
            callbacks[buttons["registration_status_other"]],
            callbacks[buttons["registration_status_t-bank"]],
        ]
    ),
...
async def sign_in_enter_status_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(
        status=status, registration_additional_message_id=additional_message.message_id
    )
...
async def sign_in_enter_status_handler(message: Message, state: FSMContext):
    ...
    if not phone_number["valid"]:
        try:
            return await bot.edit_message_text(
                chat_id=message.chat.id,
                message_id=data["registration_message_id"],
                text=LEXICON["sign_in_enter_phone_number_again"].format(
                    phone_number["reason"]
                ),
                reply_markup=kb.profile_registration_back_to_status(),
            )
    ...
    await bot.delete_message(
        message.chat.id, data["registration_additional_message_id"]
    )
    await bot.edit_message_text(
        chat_id=message.chat.id,
        message_id=data["registration_message_id"],
...
async def profile_registration_back_to_callback(
    ...
    if state_value not in (
        ...
        state_value in (UserState.default_state, None) and destination != "phone-number"
    ):
    ...
    if destination == "name":
        await callback.message.edit_text(
            ...
            reply_markup=kb.cancel_registration()
            if not data.get("registration_to_event", None)
            else None,
        )
        await state.set_state(UserState.sign_in_enter_name)
        ...
        await callback.message.edit_text(
            text=LEXICON["sign_in_enter_date_of_birth"].format(
                data["name"].split("_")[1]
            ),
            reply_markup=kb.profile_registration_back_to_name(),
        )
        ...
        await callback.message.edit_text(
            text=LEXICON["sign_in_enter_status"], reply_markup=kb.enter_status()
        )
        await bot.delete_message(
            chat_id=callback.message.chat.id,
...
@router.callback_query(
    F.data.in_(
        [
            callbacks[buttons["confirm_registration"]],
            callbacks[buttons["cancel_registration"]],
        ]
    )
)
...
async def profile_callback_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if not (user.name and user.date_of_birth):
        callback = await callback.message.edit_text(
            LEXICON["you_need_to_sign_in"]
        )  # TODO: кнопка отмены ведёт в меню

        await state.set_state(UserState.sign_in_enter_name)
        return await state.update_data(
            registration_message_id=callback.message_id, show_profile=True
        )
...
async def cancel_payment_confirmation_handler(
    callback: CallbackQuery, state: FSMContext
):
    event_id = int(callback.data.split("_")[-1])
    ...
    await callback.answer(
        "Отправить подтверждение оплаты можно в разделе мероприятия", show_alert=True
    )
...
async def profile_message_handler(message: Message, state: FSMContext):
    ...
    if not (user.name and user.date_of_birth):
        message = await message.answer(
            LEXICON["you_need_to_sign_in"], reply_markup=kb.cancel_registration()
        )

        await state.set_state(UserState.sign_in_enter_name)
        return await state.update_data(
            registration_message_id=message.message_id, show_profile=True
        )
...
async def top_up_balance_handler(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        LEXICON["top_up_balance_menu"], reply_markup=kb.coins_amount_kb()
    )
    await state.set_state(UserState.default_state)
...
async def enter_coins_amount_handler(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        top_up_message_message_id = (
            await callback.message.edit_text(
                text=LEXICON["top_up_balance_manual_input"],
                reply_markup=kb.back_to_top_up_menu(),
            )
        ).message_id

        await state.set_state(UserState.enter_coins_amount)
        return await state.update_data(
            top_up_message_message_id=top_up_message_message_id
        )
    ...
    await callback.message.edit_text(
        text=LEXICON["top_up_balance_instructions"].format(
            coins_amount,
            "" if coins_amount == 1 else "s",
            coins_amount * CUETA_COIN_PRICE,
            fundraiser.phone_number,
            fundraiser.preferred_bank,
            fundraiser.username,
        ),
    ...
    try:
        ...
        if data.get("top_up_message_message_id", False):
            try:
                return await bot.edit_message_text(
                    chat_id=message.chat.id,
                    message_id=data["top_up_message_message_id"],
                    text=LEXICON["top_up_balance_manual_input_again"],
                    reply_markup=kb.back_to_top_up_menu(),
                )
            except TelegramBadRequest:
                return
...
async def confirm_transaction(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.data.split("_")[-2] == "confirm":
        ...
        if not await db.update_transaction_status(
            transaction.id, "ready_to_confirm_transaction"
        ):
        ...
        if transaction.status == "confirmed":
            ...
            await callback.message.edit_text(
                text=LEXICON["transaction_confirmed_contact_administrator"].format(
                    fundraiser.username
                )
            )
            return print(f"\n\nERRER ID 7\nTRANSACTION ID: {transaction.id}")
...
async def send_transaction_confirmation_handler(message: Message, state: FSMContext):
    ...
    try:
        await bot.edit_message_reply_markup(
            chat_id=message.chat.id,
            message_id=data["last_transaction_confirmation_message_message_id"],
            reply_markup=None,
        )
    except Exception as e:
    ...
    if message.photo:
        await bot.send_photo(
            chat_id=transaction.fundraiser_id,
            photo=message.photo[-1].file_id,
            caption=LEXICON["fundraiser_transaction_confirmation_caption"].format(
                str(transaction.amount).rstrip("0").rstrip("."),
                user_info,
    ...
    await db.update_transaction_status(
        transaction.id, "waiting_for_fundraiser_confirmation"
    )
